...
class GRU(nn.Module):
    def __init__(self, in_channels, embedding_channels, hidden_channels, classes, num_layers, dim_reduction=False):
        ...
        self.GRU_layer = nn.GRU(input_size=tmp_channels, hidden_size=hidden_channels, num_layers=num_layers, bias=False)
        self.GRU_out_layer = nn.Sequential(
    ...
    def forward(self, x, device):
        ...
        _, ht = self.GRU_layer(x, (h_0))
        x = self.dropout_layer(ht[-1])
        x = self.GRU_out_layer(x)
        return x
...
if __name__ == '__main__':
    ...
    model = GRU(in_channels=2048, embedding_channels=1024, hidden_channels=512, classes=1, num_layers=3, dim_reduction=True)
